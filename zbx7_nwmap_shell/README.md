...
- その URL スクリプトでは、
  ...
  - 「範囲」は「手動ホスト処理」を選択。
  - 「タイプ」は「URL」。
  - 「URL」には、`ttyd` が待ち受けている URL に、第１パラメータとして監視対象ノードの IP アドレスに当たる
    `{HOST.IP} マクロを追加したものを設定。

  <img src="./09_url_script_to_telnet.png" width=60%>telnetするためのURLスクリプト<img/>

- これで、ネットワークマップ上の監視対象ノードのアイコンをクリック
  → ポップアップメニューからこのスクリプトを選択
  → 監視対象ノードへ telnet したターミナル画面がブラウザ上に開く。
