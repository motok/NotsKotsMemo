...
- snmptranslateやsnmpwalkなどを用いてRTX1300と通信できるように
  両側で設定しておく。
  - RTX1300側は、SNMPv2でReadOnlyのコミュニティを設定。
  - Zabbixサーバ側は
...
- Zabbix から SNMP で RFC1213-MIB::sysDescr.0 を取ってきて、それが
  RTX1300 のものだったら監視対象に追加する、という論理。
  - ICMP echoに応答があれば追加とかもできるけど、それだと他のノードも
    追加してしまうので、一応機種まで見ることにした。
  - 今回は SNMPv2 でコミュニティ(RO)を設定してあるので、例え RTX1300 で
...
| IPアドレスの範囲 | 指定されたIPアドレス(の範囲)について、ホストディスカバリを行う。今回はRTX1300のアドレス１個だけに限定。 |
...
| ポート | デフォルトの 161 |
| SNMPコミュニティ | RTX1300 と通信できるもの(上記のコマンドラインを参照) |
...
| 実行条件 B | 「受信した値」が "RTX1300 Rev" を含むという条件。今探しているRTX1300なら含むし、別の機種なら多分含まないであろう。 |
...
| テンプレートをリンク | 「RTX1300監視用テンプレート by SNMP」テンプレート(後述)にリンクする。 |
